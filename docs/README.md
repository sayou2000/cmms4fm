...
| [domaenen-events.md](domaenen-events.md) | Domänen-Ereignisse und Fan-out (Case E2): welche semantischen Ereignisse ein Dienst publiziert, warum es dafür einen Publisher statt fünf `publishEvent`-Aufrufe gibt (ohne Transaktion feuert kein `AFTER_COMMIT`-Listener, lautlos), warum der Fan-out **nicht** hinter `AUTOMATION_ENABLED` liegen darf, und vor allem die Grenze: ausgehende Nebenwirkungen wurden Konsumenten, fachliche Schreibvorgänge sind in der Transaktion geblieben — bis Case E1 eine Zustellgarantie liefert |
...
| [ki-readiness-architektur-analyse.md](ki-readiness-architektur-analyse.md) | KI-Readiness: zehn Infrastruktur-Cases in fünf Säulen, mit Priorisierungsmatrix und Verifikationslog. Die Landkarte, gegen die einzelne Umbauten geplant werden — E2 ist umgesetzt, siehe [domaenen-events.md](domaenen-events.md) |
| [mcp-server-konzept.md](mcp-server-konzept.md) | MCP-Server als universelles Addon: der externe, zustandslose Proxy als *eine* stabile Werkzeuggrenze zum CMMS, aus der OpenAPI-Spec abgeleitet und über Profile kuratiert — warum er neben `api/` statt darin entsteht, wie die Auth-Durchreichung dafür sorgt, dass er nie mehr kann als der vorgelegte Schlüssel, und wie Use Cases über ihn wachsen statt in ihm. **Umgesetzt als [`mcp/`](../mcp/README.md)**; Abschnitt 12 hält fest, was der Bau am Konzept korrigiert hat (Gruppen-URL der Spec, `operationId` als Tool-Name unbrauchbar, POST-Suche bricht die Lesen/Schreiben-Ableitung, Kuratierung als Voraussetzung) und welche Schritte noch fehlten. **Abschnitt 13 ist offen und der wichtigste fürs Kundengespräch**: das Profil steuert heute die ganze Instanz statt einzelner Klienten — dort stehen die drei Optionen mit ihren Kosten, was man ohne neuen Code zusagen kann, und was ein Kunde darüber hinaus verlangen wird |
